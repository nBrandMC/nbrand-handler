...
execute if entity @s[gamemode=creative] run tellraw @a ["",{text:"Transferring ",color:"gray"},{selector:"@s"},{text:" to the ",color:"gray"},{text:"nBrand Creative Server",color:"dark_red",click_event:{action:"run_command",command:"trigger transfer"},hover_event:{action:"show_text",value:[{text:"Click here to swap servers",color:"gray"}]}}]
